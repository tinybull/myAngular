describe('$q', function () {
    ...
    it("can create a Deffered", function () {
        var d = $q.defer();
    ...
    it("can resolve a promise", function (done) {
        ...
        setTimeout(function () {    //Promise callbacks are not invoked immediately.
    ...
    it('waits on promise given to resolve', function() { var d = $q.defer();
        var d2 = $q.defer();
        var fulfilledSpy = jasmine.createSpy();
        d.promise.then(fulfilledSpy);
        d2.resolve(42);
    ...
    it('rejects when promise returned from handler rejects', function() { var d = $q.defer();
        var rejectedSpy = jasmine.createSpy();
        d.promise.then(function() {
            var d2 = $q.defer(); d2.reject('fail'); return d2.promise;
        }).catch(rejectedSpy);
        d.resolve('ok');
        $rootScope.$apply();
        expect(rejectedSpy).toHaveBeenCalledWith('fail');
