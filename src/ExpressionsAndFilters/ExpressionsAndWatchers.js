...
function $ParseProvider() {
    ...
    /**
     * It takes an Angular expression string and returns a function
     * that executes that expression in a certain context
     */
...
AST.LogicalExpression = 'LogicalExpressoin';
...
ASTCompiler.prototype.compile = function (text) {
    ...
    var that = this;    //TODO: 打印解析后的函数可以看到，有些变量被覆盖了
    _.forEach(this.state.filters, function (value) {
        _.forEachRight(that.state.vars, function (val, index) {
            if (value === val) {
                that.state.vars.splice(index, 1);
            }
        });
    });
    ...
    var fn = new Function(
        ...
        ensureSafeFunction,
        ifDefined,
        this.filter);  //这个函数是调用parse服务返回的结果，函数体为动态解析的
    ...
    switch (ast.type) {
        ...
        case AST.CallExpression:
            ...
            _.forEach(ast.arguments, function (arg) {
                markConstantExpressions(arg,$filter);
        ...
        case AST.LogicalExpression:
            ...
            break;
        case AST.ConditionalExpression:
            markConstantExpressions(ast.test,$filter);
            markConstantExpressions(ast.consequent,$filter);
            markConstantExpressions(ast.alternate,$filter);
