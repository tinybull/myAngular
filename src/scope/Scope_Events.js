    ...
    this.$get = ['$parse', function ($parse) {
        ...
        Scope.prototype.$evalAsync = function (expr) {
            ...
            if (!self.$$phase && !self.$$asyncQueue.length) {
                setTimeout(function () {
                    if (self.$$asyncQueue.length) { //多次调用$evalAsync方法,此方法分在digest循环中调用,和在digest循环外调用️
                        self.$root.$digest();
                    }
                }, 0);
            }
        ...
        Scope.prototype.$new = function (isolated, parent) {    //如果传递的parent不是Scope类型的？
            ...
            //child scope自己的属性
            child.$parent = parent;     //hierarchical parent
            child.$$watchers = [];
            child.$$children = [];
            child.$$listeners = {};
        ...
        Scope.prototype.$destroy = function () {
            this.$broadcast('$destroy');    //广播Scope被移除，当前Scope及其子Scope都被移除
            if (this.$parent) {
        ...
        Scope.prototype.$emit = function (eventName) {
            ...
            var event = {       // the same exact event object passed to the hierarchy
                name: eventName,
                targetScope: this,
            ...
            var scope = this;
            do {
                event.currentScope = scope;       //attach currentScope
        ...
        Scope.prototype.$broadcast = function (eventName) {
            ...
            var listenerArgs = [event].concat(_.tail(arguments));
            this.$$everyScope(function (scope) {        //depth-first遍历
